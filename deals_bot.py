...
CHANNEL_ID = "@your_channel_or_chat_id"  # Where scheduled/fetched deals are posted
...
async def fetch_and_post_deal(
    ...
    """Simulates multi-platform scraping, dynamic title generation, and price history mapping."""
    # Production note: Integrate Playwright/Selenium or official APIs (PA-API for Amazon, Flipkart Affiliate API) here.

    # Dynamic Title Logic based on category/platform
    dynamic_titles = {
    ...
    product_title = (
        f"Sample Branded {subcategory} Item - High Performance Edition"
    )
...
async def cmd_start(message: Message):
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔍 Fetch Deals Now", callback_data="flow_fetch"
                )
            ],
            [
                InlineKeyboardButton(
                    text="⏰ Schedule Category Push", callback_data="flow_schedule"
                )
            ],
        ]
    )
...
# --- FLOW 1: CATEGORY > CATALOGUE & RATING SEARCH ---
...
@router.callback_query(
    DealSearchStates.selecting_platform, F.data.startswith("plat_")
)
...
@router.callback_query(
    DealSearchStates.selecting_category, F.data.startswith("cat_")
)
...
@router.callback_query(
    DealSearchStates.selecting_subcategory, F.data.startswith("sub_")
)
async def process_subcategory_selection(
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="⭐ 4.0 & above", callback_data="rate_4.0"
                ),
                InlineKeyboardButton(
                    text="⭐ 4.5 & above", callback_data="rate_4.5"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="⭐ Any Rating", callback_data="rate_3.0"
                )
            ],
        ]
    )
...
@router.callback_query(
    DealSearchStates.selecting_rating, F.data.startswith("rate_")
)
async def process_final_deal_fetch(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "🔄 Scanning platform catalog & applying filters..."
    )

    # Execute fetch/post simulation
...
# --- FLOW 3: SCHEDULE TIMINGS FOR CATEGORY SELECTION ---
...
@router.callback_query(
    ScheduleStates.selecting_category, F.data.startswith("schedcat_")
)
async def process_schedule_category(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Every 1 Hour", callback_data="time_1h"
                ),
                InlineKeyboardButton(
                    text="Every 6 Hours", callback_data="time_6h"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="Every 12 Hours", callback_data="time_12h"
                ),
...
@router.callback_query(
    ScheduleStates.selecting_time, F.data.startswith("time_")
)
async def process_schedule_complete(callback: CallbackQuery, state: FSMContext):
    ...
    # Map interval codes to APScheduler triggers
